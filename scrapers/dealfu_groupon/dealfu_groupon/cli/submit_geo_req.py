...
def submit_geo_request_addresses(app):
    ...
    while results:
        for r in results:
            ...
            for addr in get_in(item, "merchant","addresses"):
                #logger.info("Check address : {}".format(addr))
                if needs_address_geo_fetch(addr):
                    logger.debug("Submit geo {} for addr : {}".format(item_id, addr))
                    #logger.debug("Settings : {}".format(from_obj_settings(settings.settings_module)))
                    submit_geo_request.delay(from_obj_settings(settings.settings_module),
                                             item_id)
